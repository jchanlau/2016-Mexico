...
% General parameters
base_periods = 21;   % end of 2017
T = 12;              % remainder periods until 2018
...
IndProdGrowthAnnum = 0.01;         % 0.02 if growing
...
% 2017 40.09
WTI2016 = 34.04;
%Xbase(end,2) = Xorig(end,2)*(1-nanstd(dY_us12(:,2)));
stdWTI = -nanstd(dY_us12(:,2));
shockfactor = 1;
shock = shockfactor*stdWTI;
step = (1+shock)^(1/base_periods);
Xbase(:,2) = cumprod(repmat(step,base_periods,1))*Xorig(end,2);

% X(:,3) US 3-month yield
% 2016 100 bps hike
shock = 2;    % 1 if 100 bps hike
...
stdshock = 2;  % standard deviation shock, all except IGAE
stdIGAE  = 3;  % standard deviation shock, IGAE
...
if write_results, 
    ...
    % Simulation paths, 12-mo growth rates

    outfile = 'MXSevereGrowth.xlsx';
    for i = 1:nMX,
        idx_var = i;
        var_mx = squeeze(Ysim(:,idx_var,:));
        h = exp(cumsum(var_mx))*Yorig(end,idx_var);
